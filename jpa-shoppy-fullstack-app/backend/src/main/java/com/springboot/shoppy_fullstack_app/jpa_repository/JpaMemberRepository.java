...
// --> 서버 부팅시 컨테이너에 로딩
//interface라 이걸 구현하는 class가 필요한데 이걸 JPA가 해준다는 뜻. 서버 부팅시 컨테이너에 로딩해주는것 까지도.
@Repository
public interface JpaMemberRepository extends JpaRepository<Member, String> {
    // 주석처리해도 부모꺼를 가지고와서 정상적으로 동작한다. (문서에 save가 정의되어있음)
//    Member save(Member member); //⭐⭐ 생략 ⭕ : 상속한 부모 인터페이스에 save 메소드가 존재
...
//    Long countById(@Param("id") String id);
    ...
    //로그인 = 엔티티가 아닌 다른 객체로 결과를 출력하는 경우 "new 패키지 풀 주소 출력 객체명"
    //      형식으로 컬럼리스트를 작성!!!
